#!/usr/bin/env python3
"""
odom_base_publisher node (v2 - Minor Optimization)
...
Changes from v1:
  - Pre-allocated output Odometry message (reused each callback)
  - Pre-allocated TransformStamped messages
...
class OdomBasePublisherNode(Node):
    ...
    def __init__(self):
        ...
        # Pre-allocated messages (reused each callback)
    ...
    def _odom_cb(self, msg: Odometry):
        ...
        # First message: store initial pose inverse
        ...
        # Publish odom → base_link TF (reuse message)
    ...
    def _pose_cb(self, msg: PoseStamped):
        ...
        # Publish map → odom (reuse message)
    ...
    # ======================================================================
    # Re-publish odometry in base_link frame (reuses message)
    # ======================================================================
    ...
    def _publish_base_odom(self, zed_msg: Odometry):
        ...
        # Copy twist (ZED reports in body frame)
        out.twist.twist.linear.x = zed_msg.twist.twist.linear.x
        out.twist.twist.linear.y = zed_msg.twist.twist.linear.y
        out.twist.twist.linear.z = zed_msg.twist.twist.linear.z
        out.twist.twist.angular.x = zed_msg.twist.twist.angular.x
        out.twist.twist.angular.y = zed_msg.twist.twist.angular.y
        out.twist.twist.angular.z = zed_msg.twist.twist.angular.z

        # Copy covariance (these are arrays, direct assignment is fine)
