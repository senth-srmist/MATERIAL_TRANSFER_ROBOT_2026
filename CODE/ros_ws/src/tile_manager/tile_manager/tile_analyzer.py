...
def calculate_overlap(bounds1, bounds2):
    ...
    if x_overlap_min < x_overlap_max and y_overlap_min < y_overlap_max:
        return {
            ...
            'area': round((x_overlap_max - x_overlap_min) * (y_overlap_max - y_overlap_min), 2),
        }
    return None
    ...
    for i in range(len(tile_ids)):
        for j in range(i + 1, len(tile_ids)):
            ...
            if overlap:
                ...
                switch_i_to_j[0] = round(min(switch_i_to_j[0], overlap['x_max'] - 0.1), 2)
                switch_j_to_i[0] = round(max(switch_j_to_i[0], overlap['x_min'] + 0.1), 2)
...
def visualize_tiles(tiles, overlaps, output_path='tile_layout.png'):
    ...
    for (ti, tj), data in overlaps.items():
        for sp in [data['switch_i_to_j'], data['switch_j_to_i']]:
            ...
            draw.ellipse([px-5, py-5, px+5, py+5], fill=(0, 200, 0), outline=(0, 100, 0))
    ...
    draw.rectangle([10, legend_y + 18, 25, legend_y + 28], fill=(255, 100, 100, 80), outline=(255, 0, 0))
...
def main():
    if len(sys.argv) > 1:
        ...
        candidates = ['.', './maps', '../maps', '/workspace/ros_ws/src/tile_manager/maps']
        maps_dir = None
        for c in candidates:
    ...
    viz_path = maps_path / 'tile_layout.png'
