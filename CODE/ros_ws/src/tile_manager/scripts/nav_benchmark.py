...
class NavBenchmark(Node):
    def __init__(self, mode='tiles', duration=120):
        ...
        self.get_logger().info(f'NavBenchmark initialized - Mode: {mode}, Duration: {duration}s')
    ...
    def map_callback(self, msg):
        ...
        if self.current_map_origin is not None and new_origin != self.current_map_origin:
            ...
            self.get_logger().info(f'Tile switch detected: {self.current_map_origin} -> {new_origin}')
    ...
    def start(self):
        ...
        self.samples = []
        self.tile_switches = []
        self.get_logger().info(f'Benchmark started - {self.mode} mode for {self.duration}s')
    ...
    def get_results(self):
        ...
        costmap_values = [s['costmap_cells'] for s in self.samples if s['costmap_cells'] > 0]
        ...
        results = {
            ...
            'summary': {
                ...
                'costmap_cells': {
                    'mean': sum(costmap_values) / len(costmap_values) if costmap_values else 0,
...
def main():
    ...
    parser.add_argument('--mode', choices=['whole', 'tiles'], default='tiles',
    ...
    print(f"Costmap Mean:   {summary.get('costmap_cells', {}).get('mean', 0):,.0f} cells")
