 ...
 * 2. Checks trigger zones from tiles_config.yaml
...
private:
  ...
  bool loadConfig(const std::string & path)
  {
    try {
      ...
      // Parse trigger zones
      if (config["trigger_zones"]) {
        for (const auto & zone : config["trigger_zones"]) {
          TriggerZone tz;
          auto bounds = zone["bounds"];
          tz.x_min = bounds[0].as<double>();
          tz.x_max = bounds[1].as<double>();
          tz.y_min = bounds[2].as<double>();
          tz.y_max = bounds[3].as<double>();
          tz.from_tile = zone["from_tile"].as<int>();
          tz.to_tile = zone["to_tile"].as<int>();
          tz.heading = zone["heading"].as<std::string>();
          trigger_zones_.push_back(tz);
        }
      }
